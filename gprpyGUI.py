...
class GPRPyApp:
    ...
    def __init__(self,master):
        ...
        # pick zero time
        PickZeroTimeButton = tk.Button(
            text="Pick t0", fg="black",
            command=lambda : [self.pickZeroTime(proj),
                              self.plotProfileData(proj,fig=fig,a=a,canvas=canvas)])
        PickZeroTimeButton.config(height = 1, width = halfwid)         
        PickZeroTimeButton.grid(row=3, column=rightcol, sticky='nsew')    
        self.balloon.bind(PickZeroTimeButton,
                          "Pick time zero for a chosen trace \n" 
        ...
        SetZeroTimeButton = tk.Button(
            text="set t0", fg="black",
            command=lambda : [self.setZeroTime(proj),
                              self.plotProfileData(proj,fig=fig,a=a,canvas=canvas)])
        SetZeroTimeButton.config(height = 1, width = halfwid)         
        SetZeroTimeButton.grid(row=3, column=rightcol+1, sticky='nsew')    
        self.balloon.bind(SetZeroTimeButton,
                          "Set the two-way travel time \n" 
                          "that corresponds to the surface.\n"
                          "/!\ Pick zero time first with button Pick")
        ...
        # pickFrequenciesButton
        pickFrequenciesButton = tk.Button(
            text="Pick f", fg="black",
            command=lambda : [self.pickFrequencies(proj),
                              self.plotProfileData(proj,fig=fig,a=a,canvas=canvas)])
        pickFrequenciesButton.config(height = 1, width = halfwid)         
        pickFrequenciesButton.grid(row=14, column=rightcol, sticky='nsew')
        self.balloon.bind(pickFrequenciesButton,
        ...
        filterButton = tk.Button(
            text="bandpass", fg="black",
            command=lambda : [self.bandpass(proj),
                              self.plotProfileData(proj,fig=fig,a=a,canvas=canvas)])
        filterButton.config(height = 1, width = halfwid)         
        filterButton.grid(row=14, column=rightcol+1, sticky='nsew')
        self.balloon.bind(filterButton,
                          "Bandpass filter\n"
    ...
    def pickZeroTime(self,proj):
        trace_pour_afficher = sd.askfloat("Input","Number of trace to print")
        proj.pick_time_zero(trace_pour_afficher)
    ...
    def setZeroTime(self,proj):
        ...
        # trace_pour_afficher = sd.askfloat("Input","Trace number to print")
        # proj.pick_time_zero(trace_pour_afficher)
        # newZeroTime = sd.askfloat("Input","New zero time")
        proj.setZeroTime()
        
    def bandpass(self, proj):
        trace_pour_afficher = sd.askfloat("Input","Number of trace to print")
        proj.bandpass(trace_pour_afficher)
        
    def pickFrequencies(self,proj):
        trace_pour_afficher = sd.askfloat("Input","Number of trace to print")
        proj.pick_frequencies(trace_pour_afficher)
    ...
    def loadData(self,proj):
        filename = fd.askopenfilename( filetypes= (("All", "*.*"),
                                                   ...
                                                   ("MALA header","*.rad")))
        if filename:
            marker_existance = sd.askstring("'yes' or 'no'","Marker file exists ?")
            
            if marker_existance == 'yes' :
                marker = fd.askopenfilename() #ajout aline : fichier marqueur pour la position
                proj.importdata(filename=filename, marker = marker)
            else :
