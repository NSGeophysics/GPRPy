import sys
sys.path.insert(0, "/home/aline/GPRPY_test/gprpy")
import os
import numpy as np
...
import gprpy.toolbox.readmrq as readmrq
#from onclick_event import onclick
#from scipy.fftpack import rfft, irfft, fftfreq
from scipy import signal,fftpack
import matplotlib.pyplot as plt
import gprpy.toolbox.FilterFunctions as filters
...
class gprpyProfile:
    ...
    def __init__(self,filename=None):
        ...
        # # Initialisation for onclick
        # self.frequencies_ = []
        
        if filename is not None:
            self.importdata(filename)                 
        
    def importdata(self,filename, marker = None):
        '''
        ...
        if file_ext==".DT1" or file_ext==".HD" or file_ext==".dt1" or file_ext==".hd":
            ...
            self.freq_ech = self.info["N_pts_per_trace"]/(self.info["Total_time_window"]-self.info["TZ_at_pt"])*10E9
            # Initialize previous
            self.initPrevious()
            ...
            if marker:
                # self.profilePos = self.info["Startposition (ns) : "]+np.linspace(0.0,
                #                                                         self.data.shape[1]/self.info["Scans per second : "],
                #                                                         self.data.shape[1])
                #marker = input('marker path and file name : ')
                self.profilePos = readmrq.read_mrk(marker, self.data.shape[1])
                print(self.profilePos)
            
            elif self.info["Scans per meter : "] != 0:
                self.profilePos = np.linspace(0.0,
                                              self.data.shape[1]/self.info["Scans per meter : "],
                                              self.data.shape[1])
                print('datashape[1]', self.data.shape[1])
                
            else : 
                print("PROFIL LENGHT UNKNOWN, PLEASE CREATE A MARKER FILE")
                
            self.twtt = np.linspace(0,self.info["Length of trace (ns) : "],self.info["Samples per traces :"])
            ...
            #sample frequency in GHz
            self.freq_ech = self.info["Sample frequency (GHz) : "]
            ...
            #### @author: aline
            #read info for readdzt
            for i in self.info:
                print(i, self.info[i])
            print ('Number of traces : ', self.data.shape[1])
            ####
            ...
            self.twtt_pretopo = None
            #frequence d'echantillonage en Hz
            self.freq_ech = int(self.info["lines"])/float(self.info["time_window"])*10E9
            # Initialize previous
            self.initPrevious()
            ...
            self.twtt_pretopo = None
            #frequence d'echantillonage en Hz
            self.freq_ech = int(self.info["SAMPLES"])/float(self.info["TIMEWINDOW"])*10E9
            # Initialize previous
            self.initPrevious()
        ...
        #sauvegarde de la liste des tps et des données pour pick frequencies (graphe)
        self.twtt_before_set_zero = self.twtt
        self.data_before_set_zero = self.data
    ...
    def give_info(self):
        '''
        @author: aline
        prints info read in file header, only for DZT format
        Remplaced in the 'import data' function
        Parameters
        ----------
    ...
    def undo(self):
        ...
        '''
        self.frequencies_ = self.previous["frequencies"]
        self.t0_ = self.previous["time_zero"]
    ...
    def initPrevious(self):
        ...
        '''
        self.previous["frequencies"] = []
        self.previous["time_zero"] = self.t0_
    ...
    def pick_frequencies(self, trace_pour_afficher):
        """
        @author: aline
        Plot FFT of a chosen trace given in parameter
        Click twice in the plot to chose cutting frequencies
        To use before filtering with bandpass filter

        Parameters
        ----------
        trace_pour_afficher : TYPE int
            DESCRIPTION. the number of the trace to plot

        Returns
        -------
        self.frequencies_ : TYPE list
            DESCRIPTION. [fmin, fmax] for bandpass filter

        """
        #x = self.twtt_before_set_zero
        #y = self.data_before_set_zero.transpose()[0]
        x = self.twtt
        y = self.data.transpose()[0]

        num = len(x)
        N = filters.nextpow2(num)
        dt = x[1]-x[0]
        df = 1/(N*dt)
        Nyq = 1/(2*dt)
                    
        fpos = np.linspace(0,Nyq-df,N//2)
        #fpos = np.linspace(0,Nyq-df,num//2)
        #fpos2 = np.linspace(0,2*Nyq-df,N)
        #fpos3 = np.linspace(-Nyq,Nyq-df,N)
        
        A = fftpack.fft(y,N).tolist()[0]
        A_shift = fftpack.fftshift(A)
            
        #Xpos = A_shift[n:]
        Xpos = A_shift[N//2:]
        
        fig, ax = plt.subplots()  
        plt.plot(fpos,abs(np.real(Xpos)))
        plt.xlabel("Frequency (GHz)")
        plt.title('FFT trace %i' %trace_pour_afficher)
        #plt.plot(fpos2,A)
        #plt.plot(fpos3, A_shift)
        
        
        def onclick(event):
            #global frequencies
            #pick des fréquences en GHz
            print('frequence (GHz): ',  event.xdata)
            self.frequencies_.append(event.xdata)
        
            if len(self.frequencies_)==2 :
    ...
    def pick_time_zero(self, trace_pour_afficher):
        """
        @author: aline
        Plot a trace (number of trace given in parameter)
        Click on the signal to chose time zero
        To use before doing 'set time zero'
        
        Parameters
        ----------
        trace_pour_afficher : TYPE int
            DESCRIPTION. numéro de trace à afficher
        ...
        """
        #trace_pour_afficher = int(input('Numéro de la trace à afficher : '))
        x = self.twtt
        y = self.data.transpose()[int(trace_pour_afficher-1)]
    ...
    def bandpass(self, trace_pour_afficher):
        """
        @author: aline
        Butterworth bandpass filter
        To use after picking cutting frequencies (with pick_frequencies)
        Plots the trace chosen before and after filtering and Butterworth filter applied
        ...
        fmin = self.frequencies_[0] #GHz
        fmax = self.frequencies_[1] #GHz

        trace_filtered = []
        for n_trace in range(0, self.data.shape[1]):
            datain_filtered, Xpos_filt, N = filters.filtre_passband_calcul(self.twtt,self.data.transpose(), fmin, fmax, self.freq_ech, 2, n_trace, trace_pour_afficher)
            trace_filtered.append(datain_filtered[0])

        #conversion de la liste d'array en matrice + transposée
        self.data = np.asmatrix(trace_filtered).transpose()
        print("data filtered")
        
        # Put in history
        histstr = "mygpr.filter(%g,%g)" %(fmin,fmax)
        self.history.append(histstr)                        
